...
\newcommand{\E}[2]{\mathbb E_{#1}\bracket{#2}}
:::
...
Take a look at this [rough implimentation](../polynomial_regression_vb).
...
We will take a polynomial approximation to $l(y_i; \psi_i) \approx \sum_k c_{ik} P_k(\psi_i) = f_i(\psi_i)$.
Then we can approximate the ELBO


```{=markdown}
\begin{align}
  F_1(q) &= \E{q}{\sum l_i(\psi_i)\} - KL(q || p) \\\\
         &\approx \E{q}{\sum f_i(\psi_i)\} - KL(q || p) = F_2(q)
\end{align}
```
